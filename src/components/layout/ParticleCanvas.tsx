...
interface Particle { x: number; y: number; ox: number; oy: number; vx: number; vy: number; }
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        particles.push({ x: ox + (Math.random() - 0.5) * 18, y: oy + (Math.random() - 0.5) * 18, ox, oy, vx: 0, vy: 0 });
      }
    };
    resize();
    ...
    const tick = () => {
      ...
      for (const p of particles) {
        ...
        ctx.fillStyle = disturbed > 0.4 ? `rgba(200,255,87,${0.5 + disturbed * 0.4})` : `rgba(245,240,232,${0.55})`;
