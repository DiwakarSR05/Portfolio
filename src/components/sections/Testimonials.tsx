...
const items = [
  {
    quote: "He is a rare combination of deep technical skill and clear communication. He helped us ship a complex feature in record time.",
    name: "Abishek", role: "CEO, ABC.co", initials: "AP",
  },
  {
    quote: "He inherited a tangled Stripe integration and within two weeks our chargeback rate had halved. Clear thinking, clear code, clear communication.",
    name: "John Doe", role: "Founder, Ecommerce", initials: "JD",
  },
      ...
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="relative min-h-[360px]">
          <AnimatePresence mode="wait">
            ...
            >
              <span className="absolute -top-8 left-6 font-display font-bold text-[200px] text-[var(--primary)] opacity-25 leading-none select-none">&ldquo;</span>
        ...
        <div className="flex items-center justify-center gap-3 mt-8">
          <button onClick={() => setI((x) => (x - 1 + items.length) % items.length)}
            className="w-10 h-10 rounded-full bg-[var(--surface)] text-[var(--ink)] clay-sm">‹</button>
          {items.map((_, k) => (
            <button key={k} onClick={() => setI(k)}
              className={`h-2 rounded-full transition-all ${k === i ? "w-8 bg-[var(--primary)]" : "w-2 bg-[var(--muted)]/40"}`} />
          ))}
          <button onClick={() => setI((x) => (x + 1) % items.length)}
            className="w-10 h-10 rounded-full bg-[var(--surface)] text-[var(--ink)] clay-sm">›</button>
        </div>
      </div>
    </section>
