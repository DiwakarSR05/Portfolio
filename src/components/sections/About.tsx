...
const philosophy = [
  { title: "Performance First", desc: "Bundles minimal. Queries indexed. Time-to-interactive measured in milliseconds, not seconds." },
  { title: "Design-Aware", desc: "Engineering with typographic eye. Pixels matter as much as packets." },
  { title: "Systems Thinker", desc: "Architecture choices outlive features. I build for the version six months from now." },
];
      ...
      <div className="max-w-[1400px] mx-auto grid lg:grid-cols-[0.42fr_0.58fr] gap-16 items-start">
        ...
        >
          <div className="bg-[var(--surface)] rounded-3xl clay-lg aspect-[4/5] overflow-hidden relative">
            ...
            <svg viewBox="0 0 400 500" className="absolute inset-0 w-full h-full mix-blend-multiply opacity-30">
              <defs>
          ...
          <div className="absolute -bottom-6 -left-6 bg-[var(--surface)] text-[var(--ink)] clay-md rounded-2xl px-5 py-4">
            <div className="font-mono text-[10px] tracking-widest text-[var(--muted-ink)]">YEARS BUILDING</div>
        ...
        >
          ...
          <h2 className="font-display font-bold leading-[1.05] mb-8 text-[var(--ink-light)]"
          ...
          <div className="space-y-5 font-body font-light text-[17px] leading-[1.75] text-[var(--muted)] mb-10 max-w-[560px]">
            <p>
              I&apos;ve spent the last two years moving fluidly between the database and the
              design canvas — most engineers pick a side; I never could. The interesting
              problems live in the seams.
            </p>
            <p>
              My favourite work happens when a SQL query rewrite drops a dashboard from
              4 seconds to 80ms, and the same week I&apos;m tuning a button&apos;s easing curve
              until it feels right in the hand. Both matter. Both ship.
            </p>
            <p>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {philosophy.map((p, i) => (
              ...
              >
                ...
                <div className="font-body text-[13px] text-[var(--muted-ink)] leading-[1.6]">{p.desc}</div>
              </motion.div>
            ))}
          </div>
        </motion.div>
