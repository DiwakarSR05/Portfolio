    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          access_key: "7c122c19-1b93-4c9b-8e7a-24791e61ea33",
          name: form.name,
          email: form.email,
    ...
    <section id="contact" className="relative bg-(--bg-2) py-32 px-6 lg:px-10">
      <div className="max-w-[1300px] mx-auto">
        <div className="mb-16 max-w-4xl">
          ...
          <h2 className="font-display font-bold leading-[0.95] text-(--ink-light)"
        ...
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-8">
          <motion.form
            ...
            viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7, ease: EASE_OUT_EXPO }}
            onSubmit={submit}
          ...
          >
            <div className="grid sm:grid-cols-2 gap-5">
              <Field label="Full Name *">
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  required className="w-full bg-surface-dim clay-inset rounded-xl px-4 py-3 outline-none focus:ring-2 ring-(--primary) font-body text-[15px]" />
              </Field>
              <Field label="Email *">
            ...
            <Field label="Subject">
              <select value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })}
                ...
                {["Job Opportunity", "Freelance", "Collaboration", "Other"].map((s) => <option key={s}>{s}</option>)}
              </select>
            </Field>
            <Field label="Message *">
            ...
            <button type="submit" disabled={state !== "idle"}
              ...
              {state === "loading" && <><span className="w-4 h-4 rounded-full border-2 border-(--ink) border-t-transparent animate-spin" /> Sending…</>}
          ...
          <motion.div
            initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7, delay: 0.1, ease: EASE_OUT_EXPO }}
            className="space-y-4"
          >
            <button onClick={copyEmail}
              ...
              <div className="font-mono text-[10px] tracking-widest text-muted-ink mb-2">EMAIL · CLICK TO COPY</div>
              <div className="font-display font-semibold text-2xl">deewakarsinghraz@gmail.com</div>
              {copied && <div className="text-xs mt-2 text-primary-muted font-mono">✓ Copied to clipboard</div>}
            </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <a key={x.l} href="#" className="bg-(--surface) text-(--ink) rounded-2xl clay-sm p-5 hover:translate-y-[-3px] transition-transform">
                  <div className="font-mono text-[10px] tracking-widest text-muted-ink mb-1.5">{x.l}</div>
            ...
            <div className="bg-(--bg) border border-(--border) rounded-3xl p-6 text-(--ink-light)">
              ...
              <div className="font-display text-lg leading-snug mb-2">Available for projects starting <span className="text-(--primary)">March 2026</span>.</div>
              <div className="font-body text-[13px] text-muted">Typical response time: under 24 hours.</div>
            </div>
          </motion.div>
        </div>
      </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block font-mono text-[10px] tracking-widest text-muted-ink uppercase mb-2">{label}</span>
      {children}
    </label>
  );
}
