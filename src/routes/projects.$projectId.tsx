        ...
        <div className="bg-(--bg) text-(--ink-light) min-h-screen">
            ...
            <section className="relative pt-32 pb-16 lg:pt-48 lg:pb-32 px-6 lg:px-10 overflow-hidden">
                <div className="max-w-[1400px] mx-auto relative z-10">
                    ...
                    >
                        <Link to="/" hash="work" className="inline-flex items-center gap-2 text-muted hover:text-(--primary) transition-colors mb-8 font-ui text-sm uppercase tracking-widest">
                            <span>←</span> Back to Portfolio
                        </Link>
                        <div className="eyebrow mb-6">// {project.category.replace("-", " ")}</div>
                        <h1 className="font-display font-bold leading-[1] mb-8"
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-10 border-y border-(--border)">
                            <div>
                                <div className="text-[10px] uppercase tracking-tighter text-muted mb-2 font-mono">Role</div>
                                <div className="font-ui text-sm">{project.role}</div>
                            </div>
                            <div>
                                <div className="text-[10px] uppercase tracking-tighter text-muted mb-2 font-mono">Year</div>
                                <div className="font-ui text-sm">{project.year}</div>
                            </div>
                            <div>
                                <div className="text-[10px] uppercase tracking-tighter text-muted mb-2 font-mono">Stack</div>
                                <div className="font-ui text-sm flex flex-wrap gap-x-2">
                                    {project.stack.slice(0, 3).join(", ")}
                                </div>
                            </div>
                            <div>
                                <div className="text-[10px] uppercase tracking-tighter text-muted mb-2 font-mono">Live</div>
                                <a href={project.links.live} target="_blank" rel="noreferrer" className="font-ui text-sm text-(--primary) hover:underline">
                                    Visit Site ↗
                                </a>
                            </div>
                        </div>
                ...
                <div className="max-w-[1400px] mx-auto grid lg:grid-cols-[1fr_1.5fr] gap-20">
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {project.stack.map((s) => (
                                <span key={s} className="px-4 py-2 rounded-full border border-(--border) text-xs font-ui bg-(--surface)/5">
                                    {s}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="space-y-12">
                        ...
                        <div className="grid md:grid-cols-2 gap-12 pt-12 border-t border-(--border)">
                            <div>
                                <h3 className="font-display font-bold text-lg uppercase tracking-widest text-(--primary) mb-6">The Challenge</h3>
                                <p className="font-body text-muted-ink leading-relaxed">{project.challenge}</p>
                            </div>
                            <div>
                                <h3 className="font-display font-bold text-lg uppercase tracking-widest text-gradient-lime mb-6">The Solution</h3>
            ...
            <section className="px-6 lg:px-10 py-32">
                <div className="max-w-[1400px] mx-auto">
                    <h2 className="font-display font-bold text-5xl mb-16 text-center">Visual <span className="text-muted">Journey</span></h2>
                    <div className="grid md:grid-cols-2 gap-10">
                        {project.screenshots.slice(1).map((src, idx) => (
                            ...
                            >
                                <img src={src} alt={`${project.title} screen ${idx + 2}`} className="w-full h-auto" />
                            </motion.div>
                        ))}
                    </div>
                </div>
